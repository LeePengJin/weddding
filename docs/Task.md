...
- [x] What if the items in the venue is booked, but the venue suddenly become unavailable. Let the vendor cancel before making it unavailable?
- [x] If the items in the venue is already sent booking request, the system shall not allow the user to submit the booking request again, unless the previous booking request is rejected. If not it will cause redundant record, where 2 booking requests if for the same item in the same venue design.
- [x] Allow pan for adjusting dimension.
